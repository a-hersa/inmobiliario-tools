...
class DatosCalculadora:
    def __init__(self, nombre, precio, metros):
            ...
            self.agencia = 0
            self.tasacion = 200
            self.total_compra = self.p_compra+self.itp+self.reforma+self.notaria+self.registro+self.agencia+self.tasacion
            self.ibi = 450
            self.basuras = 0
            self.comunidad = 400
            self.seguros = 400
            self.total_gastos = self.ibi+self.basuras+self.comunidad+self.seguros
            self.alquiler = 650
            self.financiado = 80
            self.hipoteca = (self.financiado/100)*self.p_compra
            self.a_aportar = self.p_compra+self.itp+self.reforma+self.notaria+self.registro+self.agencia+self.tasacion-self.hipoteca
            self.plazo = 30
            self.interes_anuales = 3.5/100
            self.interes_mensuales = (1+self.interes_anuales)**(1/12)-1
            ...
            self.r_bruta = round(((self.alquiler*12)/self.total_compra),2)
            self.r_neta = ((self.alquiler*12)-self.total_gastos-self.cuota_intereses)/self.total_compra
